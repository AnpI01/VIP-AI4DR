...
def clean_text(s: str) -> str:
    # Simple, clear preprocessing: remove URLs, extra whitespace, lowercase
    s = str(s)
    s = re.sub(r"http\S+", " ", s)
...
def main():
    ...
    # IMPORTANT: QProp TSV has NO header row -> use header=None
    df = pd.read_csv(p, sep="\t", header=None, low_memory=False, on_bad_lines="skip")

    # In QProp:
    # column 0 is the article text
    # last column is the label (-1/1 typically)
    text_col = 0
    ...
    # Bag-of-words counts (like the team)
